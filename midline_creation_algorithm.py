...
line_path = "C:/MagDiss_test/"
# создание базы данных для записи промежуточных и конечных результатов
# arcpy.CreateFileGDB_management(line_path, "midline_smooth.gdb")
# ЗАДАЙТЕ ПУТЬ К БАЗЕ ДАННЫХ, КУДА БУДУТ ЗАПИСАНЫ ВСЕ ФАЙЛЫ
arcpy.env.workspace = str(line_path) +  "midline_smooth.gdb"
...
fc = "C:/MagDiss_test/clipped_lines_small.shp" # набор исходных данных с горизонталями, feature class
...
Hvalues = [row[0] for row in arcpy.da.SearchCursor(fc, "Habs")]
...
arcpy.AddField_management(fc, "line_type", "TEXT", field_alias = "line_type")
...
RasToLines = arcpy.RasterToPolyline_conversion(ThinRaster, midlines, background_value = "ZERO", 
simplify = "SIMPLIFY", raster_field = "VALUE") 

# Оптимизация формы линий
# параметры подобраны эмпирически
midlines_smpl = "midlines_smpl"
SmpLines = CA.SimplifyLine(midlines, midlines_smpl, "BEND_SIMPLIFY", tolerance = 25) # диаметр сглаживания - 25 м
midlines_smooth = "midlines_smooth"
SmthLines = CA.SmoothLine(midlines_smpl, midlines_smooth, "PAEK", tolerance = 5)

arcpy.AddMessage("Отбор срединных линий") 
arcpy.MakeFeatureLayer_management(fc, "suppl_lines")  
...
intersection = arcpy.Intersect_analysis(for_inters, inters_point, "ALL", output_type = "point")
...
pts_inters_midlines = arcpy.da.SearchCursor(inters_point_single, "FID_new_lines_full")
midlines_split = arcpy.da.SearchCursor(sp_join_midl_intersect, "OBJECTID")

# Удаление - для отладки. В итоговом скрипте не нужно 
...
arcpy.CopyFeatures_management(fc, full_lns_copy)
...
arcpy.CopyFeatures_management(fc, full_lines_copy)
...
joined_cont_copy = "joined_cont_copy"
...
final_lines = "finally_processed_lns"
arcpy.MultipartToSinglepart_management(dissolved_final_lns, final_lines)

# удаление ненужных колонок с атрибутами
arcpy.DeleteField_management("finally_processed_lns", "ORIG_FID")

# создание shape-файла с новыми горизонталями
arcpy.FeatureClassToFeatureClass_conversion(final_lines, line_path, "finally_processed_lns.shp") 
